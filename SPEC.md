...
- Your engine runs in its own process: **single-threaded, no fork, no
  files, no sockets**. Memory is capped at 256 MB.
- `history` is the position hashes since the last irreversible move,
  keyed by `hfc_rules::ZOBRIST`.
- A returned move is matched on from-square, to-square and promotion
  piece; the other flag bits are advisory.
